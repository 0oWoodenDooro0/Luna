...
class DungeonCommand : Command {
    override val name = "dungeon"
    override val description = "探索你目前選中的自定義地圖"
    ...
    override suspend fun handle(interaction: ChatInputCommandInteraction) {
        ...
        if (activeMap == null) {
            interaction.deferPublicResponse().respond {
                embed {
                    title = "探索失敗"
                    description = "❌ 你目前沒有選中的地圖。請使用 `/map select <id>` 來選取一個地圖！"
                    color = dev.kord.common.Color(0xFF0000)
                }
            }
            return
        ...
        if (eventRoll < RpgConfig.Exploration.EVENT_ROLL_RESOURCE_THRESHOLD) {
            ...
            interaction.deferPublicResponse().respond {
                embed {
                    ...
                    description =
                        """
                        $username 在自定義地圖 第 $currentFloor 層探索中發現了 $foundResource x $finalAmount！
                        
                        進度：${progressionResult.finalRoomCount} / 20 房間
