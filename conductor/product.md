# Initial Concept
Discord 文字 RPG 遊戲企劃書

# Project Vision: Discord Text RPG (Luna)

## Core Concept
A simple, solo-focused Discord Text RPG where players explore endless dungeons, fight monsters, collect resources, and upgrade their equipment to reach deeper levels. The game is designed for RPG enthusiasts who enjoy statistical progression and prestige/rebirth mechanics.

## Target Audience
- **RPG Enthusiasts:** Players who enjoy item optimization, numerical growth, and long-term progression systems like rebirth/prestige.

## Setting & Theme
- **Minimalist/Generic Fantasy:** The game focuses on core gameplay mechanics over complex narrative or lore. Simple descriptions of monsters, materials, and dungeon floors are prioritized to keep the experience clean and easy to grasp.

## Key Features
- **Endless Dungeon Exploration:** A continuous exploration mode where players encounter monsters and find resources.
- **Automated Turn-Based Combat:** Tactical but fast-paced combat based on HP, ATK, DEF, and SPD stats.
- **Material & Equipment System:** Collecting Wood, Stone, and Metal to upgrade weapons and armor for permanent stat boosts.
- **Rebirth (Prestige) System:** Resetting level and equipment for permanent powerful buffs (increased resource gain, base stats, etc.).

## Player Interaction
- **Solo Play:** The current focus is a dedicated solo experience without mandatory social or competitive features.

## Integration
- **Parallel System:** The RPG system will co-exist with the existing "Undercover" game as a separate set of Discord commands (/explore, /rpg_stats, etc.).
